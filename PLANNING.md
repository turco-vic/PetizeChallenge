...
### Fase 5 — Componentes
- [x] `RepositoryCard` — exibe um repositório (nome como link, estrelas, data de atualização)
- [x] `RepositorySort` — componente Select de ordenação estilizado com Chakra UI
- [x] `UserProfile` — sidebar esquerda (avatar, nome, bio, stats, info de contato, botões)
...
- [x] `useGitHubUser(username)` — fetch user, expose `{ data, isLoading, error }`
- [x] `useInfiniteRepos(username, sort)` — pagination with IntersectionObserver sentinel

### Phase 5 — Components
- [x] `RepositoryCard` — displays single repo (name as link, stars, updated date)
- [x] `RepositorySort` — sort Select component styled with Chakra UI
- [x] `UserProfile` — left sidebar (avatar, name, bio, stats, contact info, buttons)
